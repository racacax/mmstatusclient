<template>
  <TableComponent title="Players" classes="col-12 col-lg-6" :display-page="true" :callback="callback" :data="data" :order-by="orderByList" :columns="columns" :bottom-label="lastUpdated" />
</template>
...
function fetchStats() {
  ...
  APIClient.getPlayersStatistics(orderBy.value).then((j) => {
    stats.value = j
    formatData()
  })
}
...
function formatData() {
  ...
  if(stats.value !== null) {
    ...
    stats.value.results.filter((el,i)=>i >= (page.value -1) * 10 && i < page.value * 10).forEach((e) => {
      currentData.push([
          `
          <a target="_blank" href="/#/statistics/${e.uuid}">${e.name}</a>
          `, e.played,
        e.wins.toString().replace(" ", "&nbsp;") + " (" + Math.round(e.wins * 100 / e.played).toString() + "&nbsp;%)",
        e.losses.toString().replace(" ", "&nbsp;") + " (" + Math.round(e.losses * 100 / e.played).toString() + "&nbsp;%)",
        e.mvps.toString().replace(" ", "&nbsp;") + " (" + Math.round(e.mvps * 100 / e.played).toString() + "&nbsp;%)",
      ])
    })
